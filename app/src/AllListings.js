...
export default function AllListingsPage({ key }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const unsubscribeListings = onSnapshot(collection(database, 'listings'), async (querySnapshot) => {
        const listingsData = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  
        const updatedListingsData = await Promise.all(
        ...
        const snapshot = await getDocs(query(collection(database, 'listings'), where('bidding', '==', false)));
        const filteredListingsIDs = snapshot.docs.map(doc => doc.id);
        const filteredListingsData = updatedListingsData.filter(item => filteredListingsIDs.includes(item.id));
  
        setAllListingsData(filteredListingsData);
      });
